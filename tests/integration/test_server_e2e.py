...
def test_padelflow_api_endpoints(server_fixture: subprocess.Popen[str]) -> None:
    """Test PadelFlow REST API endpoints."""
    # List tournaments
    res = requests.get(f"{BASE_URL}/api/tournaments")
    ...
    # Get details
    res = requests.get(f"{BASE_URL}/api/tournaments/{t_id}")
    assert res.status_code == 200
    assert res.json()["id"] == t_id

    # Get dashboard
    res = requests.get(f"{BASE_URL}/api/tournaments/{t_id}/dashboard")
    assert res.status_code == 200
    assert "completion_percentage" in res.json()

    # Get leaderboard
    res = requests.get(f"{BASE_URL}/api/tournaments/{t_id}/leaderboard")
    assert res.status_code == 200
